...
def transform_librispeech(
    ...
    metadata_files_names, already_exist = create_directories(
    ...
    for metadata_files_name in metadata_files_names:
        ...
        process_metadata_file(metadata_files_name, metadata_file,
                              already_exist, freqs, modes, n_src,
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            for metadata_files_name in metadata_files_names:
                ...
                os.makedirs(os.path.join(mode_path, 'metadata'),
                            exist_ok=True)
                try:
                    os.mkdir(directory_path)
                except FileExistsError:
                    already_exist.append(directory_path)
                    print(f"The {directory_name} directory already exist"
                          f" in {mode_path} files won't be overwritten")
                for subdir in subdirs:
                    # Create mixtures and sources directories
                    subdir_path = os.path.join(directory_path, subdir)
                    os.makedirs(subdir_path, exist_ok=True)

    return metadata_files_names, already_exist


def process_metadata_file(metadata_files_name, metadata_file, already_esxist,
                          freqs, modes, n_src, librispeech_root_path,
                          dataset_root_path):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            # Directory where the mixtures and sources will me stored
            directory_name = metadata_files_name.replace(
                'generating_mixture_', "").split('.')[0]
            directory_path = os.path.join(mode_path, directory_name)
            # If the files already exist then continue the loop
            if directory_path in already_esxist:
                continue
            print(f"Creating mixtures and sources from {metadata_files_name} "
                  f"in {directory_path}")
            ...
            for index, row in metadata_file.iterrows():
                ...
                add_to_mixture_metadata(mixture_df, mixture_id,
                                        ...
                                        absolute_source_path_list, length)
...
def add_to_mixture_metadata(mixture_df, mixture_id, absolute_mixture_path,
    ...
    row_mixture = [mixture_id, absolute_mixture_path] + \
                  absolute_source_path_list + [length]
