...
class MultiphaseGammatoneFB(Filterbank):
    ...
    def __init__(
        self, n_filters=128, kernel_size=16, sample_rate=8000, stride=None
    ):
        super(MultiphaseGammatoneFB, self).__init__(
            n_filters, kernel_size, stride=stride
        )
...
def generate_mpgtf(samplerate_hz, length_in_seconds, N):
    ...
    remaining_phase_pairs = ((N - np.sum(phase_pair_count) * 2) / 2).astype(
        int
    )
