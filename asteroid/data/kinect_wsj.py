...
def make_dataloaders(
    train_dir, valid_dir, n_src=2, sample_rate=16000, segment=4.0, batch_size=4, num_workers=None, **kwargs,
):
    num_workers = num_workers if num_workers else batch_size
    train_set = KinectWsjMixDataset(train_dir, n_src=n_src, sample_rate=sample_rate, segment=segment)
    ...
    def __getitem__(self, idx):
        ...
        for src in self.sources:
            if src[idx] is None:
                ...
                s, _ = sf.read(src[idx][0], start=rand_start, stop=stop, dtype='float32', always_2d=True)
